...
channelQuery() {
  ...
  then
    ...
    peerChannelGetInfo "my-channel2" "cli.org2.example.com" "peer0.org2.example.com:7061"

  elif [ "$1" = "fetch" ] && [ "$2" = "config" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org2" ] && [ "$5" = "peer0" ]; then
    TARGET_FILE=${6:-"$channel-config.json"}

    peerChannelFetchConfig "my-channel2" "cli.org2.example.com" "$TARGET_FILE" "peer0.org2.example.com:7061"

  elif [ "$1" = "fetch" ] && [ "$3" = "my-channel2" ] && [ "$4" = "org2" ] && [ "$5" = "peer0" ]; then
    BLOCK_NAME=$2
    TARGET_FILE=${6:-"$BLOCK_NAME.block"}

    peerChannelFetchBlock "my-channel2" "cli.org2.example.com" "${BLOCK_NAME}" "peer0.org2.example.com:7061" "$TARGET_FILE"
